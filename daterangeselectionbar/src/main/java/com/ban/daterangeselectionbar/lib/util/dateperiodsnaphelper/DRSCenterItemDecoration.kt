package com.ban.daterangeselectionbar.lib.util.dateperiodsnaphelper
...
class DRSCenterItemDecoration(
        @Px private val spacing: Int,
        private val isReversedAdapter: Boolean = false
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if ((adapterPosition == 0 && !isReversedAdapter) ||
                (adapterPosition == lm.itemCount - 1 && isReversedAdapter)) {
        ...
        } else if ((adapterPosition == lm.itemCount - 1 && !isReversedAdapter) ||
                (adapterPosition == 0 && isReversedAdapter)) {
            // Invalidate decorations when this view width has changed
